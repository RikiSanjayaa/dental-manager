...
- [x] 13.1 On the running host dev stack (or fresh docker QA stack): as doctor, verify Dashboard (KPIs, month comparison chart, recent rows), Fee Dokter Saya restyled recap (prominent total + compact component grid), Audit Akun shows login/export entries, no "Riwayat Perawatan Saya" anywhere; admin/operator regression spot check; record PASS/FAIL per step. QA phase-2 (qa profile, host dev): STEP 0-3, 5 PASS — dashboard KPIs/chart (2026-09 vs 2026-08), recap hierarchy identical to Payroll Saya (same payroll-total-card/payroll-summary-grid classes), PDF/XLSX exports valid, audit log own-only, nav exact, op.test/admin regression PASS. STEP 4 one FAIL: /my-treatment-history rendered blank (no router catch-all) — fixed by adding `path="*"` redirect to "/" (commit 23bb7c6). Post-fix desktop layout nits (dashboard bottom-row overflow) also fixed (b838929, a0f51b4).
